...
class InformationRetrievalSystem(object):
    ...
    def calculate_precision(self, result_list: list[tuple]) -> float:
        relevant_documents = sum(
            1 for score, document in result_list if hasattr(document, 'relevant') and document.relevant)
        total_retrieved_documents = len(result_list)
        precision = relevant_documents / total_retrieved_documents if total_retrieved_documents > 0 else 0
        return precision

    def calculate_recall(self, result_list: list[tuple]) -> float:
        relevant_documents = sum(
            1 for score, document in result_list if hasattr(document, 'relevant') and document.relevant)
        total_relevant_documents = sum(1 for score, document in result_list if hasattr(document, 'relevant'))
        recall = relevant_documents / total_relevant_documents if total_relevant_documents > 0 else 0
        return recall
